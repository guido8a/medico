...
<table class="table table-bordered table-hover table-condensed">
    ...
    <g:if test="${movimientos}">
        <g:each in="${movimientos}" var="gnra" status="i">
            <tr style="width: 100%; background-color: ${(gnra?.debeHaber == 'D')?'#D4E6FC':'#FFCC99'} !important; " class="movimiento">
                <td style="width: 34%">${gnra?.cuenta?.numero}<span style="font-size: 12px"> (${gnra?.cuenta?.descripcion})</span></td>

                <td class="largo" style="width: 9%;">
                    <g:textField type="number" name="porcentaje" id="por_${gnra?.id}" class="validacionNumero form-control" value="${gnra.porcentaje ?: 0}" />
                </td>
                <td class="largo" style="width: 9%;"><g:textField type="number" name="porcentaje" id="sin_${gnra?.id}" class="validacionNumero form-control"
                                                                  value="${gnra.baseSinIva ?: 0}" /></td>
                <td class="largo" style="width: 8%;"><g:textField type="number" name="impuestos" id="imp_${gnra?.id}"
                                               class="validacionNumero form-control" value="${gnra.porcentajeImpuestos?:0}" />
                </td>
                <td class="largo" style="width: 8%;"><g:textField type="number" name="ice" id="ice_${gnra?.id}"
                                               class="validacionNumero form-control" value="${gnra.ice?:0}" />
                </td>
                <td class="largo" style="width: 8%;"><g:textField type="number" name="flete" id="fl_${gnra?.id}"
                                               class="validacionNumero form-control" value="${gnra?.flete?:0}" />
                </td>
                <td class="largo" style="width: 8%;"><g:textField type="number" name="valor" id="val_${gnra?.id}"
                                               class="validacionNumero form-control" value="${gnra?.valor?:0}" />
                </td>
                <td style="width: 6%;"><g:textField type="number" name="debeHaber" id="dh_${gnra?.id}"
                                  class="form-control mayusculas" value="${gnra?.debeHaber}" />
                </td>
                <td style="width: 10%">
...
<script type="text/javascript">
    ...
    $(".btnEliminarMovi").click(function () {
        ...
        bootbox.dialog({
            ...
            message : "<strong style='font-size: 14px'> Está seguro que desea eliminar esta cuenta?. </stro>" ,
            buttons : {
                cancelar : {
                    label     : "Cancelar",
