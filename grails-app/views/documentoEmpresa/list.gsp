...
%{--    <g:if test="${session.perfil.nombre == 'Administrador'}">--}%
    <g:if test="${documentoEmpresaInstanceList.size() > 0}">
...
%{--            <g:if test="${documentoEmpresaInstance.empresa.id == session.empresa.id}">--}%
                <tr data-id="${documentoEmpresaInstance.id}" data-usado="${sri.Proceso.findByDocumentoEmpresa(documentoEmpresaInstance)?.id > 0}">
                    ...
                    <td class="derecha">${documentoEmpresaInstance?.establecimiento?.numero + " - " + documentoEmpresaInstance?.establecimiento?.nombre}</td>
                    ...
                    <td style="text-align: center">
                        <a href="#" data-id="${documentoEmpresaInstance.id}" class="btn btn-success btn-xs btn-sm btnEditar" title="Editar">
                            <i class="fa fa-edit"></i>
                        </a>
                        <a href="#" data-id="${documentoEmpresaInstance.id}" class="btn btn-danger btn-xs btnBorrar" title="Eliminar">
...
%{--            </g:if>--}%
    ...
    <g:else>
        <tr style="text-align: center">
            <td class="alert alert-warning" colspan="9"><i class="fa fa-exclamation-triangle fa-2x text-info"></i> <strong style="font-size: 16px"> No existen registros </strong></td>
        </tr>
    </g:else>
%{--    </g:if>--}%
%{--    <g:else>--}%
...
<script type="text/javascript">
    ...
    function deleteRow(itemId) {
        bootbox.dialog({
            ...
            message : "<p>¿Está seguro que desea eliminar el libretín seleccionado? Esta acción no se puede deshacer.</p>",
            buttons : {
                cancelar : {
                    label     : "Cancelar",
