    ...
    <g:each in="${sucursales}" var="sucursal">
        <tr>
            ...
            <td style="text-align: center">
                <a href="#" class="btn btn-xs btn-success btnEditar" data-id="${sucursal?.id}"
                   data-num="${sucursal?.numero}" data-nom="${sucursal?.nombre}" data-dir="${sucursal?.direccion}"><i class="fa fa-edit"></i></a>
                <a href="#" class="btn btn-xs btn-danger btnBorrar" data-id="${sucursal?.id}"><i class="fa fa-trash"></i></a>
            </td>
        </tr>
    </g:each>
...
<script type="text/javascript">
    ...
    $(".btnBorrar").click(function () {
        ...
        bootbox.dialog({
            ...
            message : "<p>¿Está seguro que desea eliminar esta sucursal? Esta acción no se puede deshacer.</p>",
            buttons : {
                cancelar : {
                    label     : "Cancelar",
